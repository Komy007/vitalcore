...
const App: React.FC = () => {
  ...
  const [viewedReportCount, setViewedReportCount] = useState(0);
  const [selectedReport, setSelectedReport] = useState<any | null>(null);

  // Report Writer State
  const [isReportModalOpen, setIsReportModalOpen] = useState(false);
  const [newReport, setNewReport] = useState({ title: '', content: '', key_point: '', image_url: '' });
  ...
  const handlePostReport = async () => {
    ...
    try {
      await api.health.create({
        ...
        key_point: newReport.key_point,
        image_url: newReport.image_url,
        summary: newReport.content.substring(0, 100) + '...'
      });
      setIsReportModalOpen(false);
      setNewReport({ title: '', content: '', key_point: '', image_url: '' });
      fetchHealthReports();
  ...
  useEffect(() => {
    fetchHealthReports();
    const views = parseInt(localStorage.getItem('health_views') || '0');
    setViewedReportCount(views);
  }, []);
  ...
  const handleReadReport = async (report: any) => {
    ...
    if (viewedReportCount < 3) {
      ...
      setSelectedReport(report);
      alert(`Guest Preview: ${newCount}/3 free articles used.`);
    } else {
  ...
  const researchData: ResearchData[] = [
    { category: 'Pancreatic (Months)', control: 14.5, phellinus: 26.8 },
    { category: 'Stomach (Months)', control: 18.2, phellinus: 29.5 },
    { category: 'Colon (Months)', control: 22.0, phellinus: 34.2 },
  ];
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const { token, user } = await api.auth.register({
        email: regEmail, password: regPw, name: regName, phone: regPhone, country: regCountry
      });
      login(token, user);
      setIsAuthModalOpen(false);
    } catch (err: any) {
  ...
  const handlePostQuestion = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.qna.create({ title: qnaTitle, content: qnaContent, is_secret: qnaSecret });
      setIsQnaModalOpen(false);
      setQnaTitle(''); setQnaContent('');
      fetchQuestions();
  ...
  const benefitIcons = [Shield, Droplet, Activity, Heart, Brain];

  return (
    <div className="font-sans text-stone-200 bg-stone-950 overflow-x-hidden min-h-screen">
      {/* Admin Panel */}
      {isAdminPanelOpen && (
        <div className="fixed inset-0 z-[80] bg-black/95 backdrop-blur-xl p-10 overflow-y-auto animate-in fade-in">
          <div className="max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-10">
              <h2 className="text-3xl font-serif font-bold text-white">Admin Dashboard</h2>
              <button onClick={() => setIsAdminPanelOpen(false)} className="p-2 bg-white/10 rounded-full hover:bg-white/20"><X /></button>
            </div>
            <div className="bg-stone-900 rounded-3xl p-8 border border-white/10">
              <h3 className="text-xl font-bold mb-6 text-amber-500">Registered Users</h3>
              <table className="w-full text-left text-stone-300">
                <thead><tr className="border-b border-white/10 text-stone-500 uppercase text-xs tracking-wider"><th className="pb-4">Name</th><th className="pb-4">Email</th><th className="pb-4">Role</th><th className="pb-4">Action</th></tr></thead>
                <tbody>
                  {usersList.map(u => (
                    <tr key={u.id} className="border-b border-white/5">
                      <td className="py-4">{u.name}</td>
                      <td className="py-4">{u.email}</td>
                      <td className="py-4"><span className={`px - 2 py - 1 rounded text - xs font - bold uppercase ${u.role === 'admin' ? 'bg-red-900 text-white' : 'bg-stone-800'} `}>{u.role}</span></td>
                      <td className="py-4">
                        {u.role !== 'admin' && <button className="text-red-500 hover:text-red-400 text-sm font-bold" onClick={async () => {
                          if (confirm('Delete user?')) { await fetch(`http://localhost:3001/api/users/${u.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }); loadAdminData(); }
                        }}> Delete</button >}
                      </td >
                    </tr >
                  ))}
      ...
      {/* Q&A Modal */}
      {
        isQnaModalOpen && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <div className="bg-stone-900 rounded-[2rem] p-8 w-full max-w-lg border border-white/10 shadow-2xl">
              <h3 className="text-2xl font-serif font-bold text-white mb-6">Ask Question</h3>
              <form onSubmit={handlePostQuestion} className="space-y-4">
                <input className="w-full p-4 bg-stone-800 border-white/10 rounded-xl text-white" placeholder="Title" value={qnaTitle} onChange={e => setQnaTitle(e.target.value)} required />
                <textarea className="w-full p-4 bg-stone-800 border-white/10 rounded-xl text-white h-32" placeholder="Start typing..." value={qnaContent} onChange={e => setQnaContent(e.target.value)} required />
                <label className="flex items-center gap-3 text-stone-400 cursor-pointer">
                  <input type="checkbox" checked={qnaSecret} onChange={e => setQnaSecret(e.target.checked)} className="w-5 h-5 accent-amber-600 rounded" />
                  <span>Secret (Only visible to you and admin)</span>
                </label>
                <div className="flex gap-4 pt-4">
                  <button type="button" onClick={() => setIsQnaModalOpen(false)} className="flex-1 py-3 text-stone-500 font-bold">Cancel</button>
                  <button type="submit" className="flex-1 py-3 bg-amber-600 text-white font-bold rounded-xl">Post</button>
                </div>
              </form>
            </div>
          </div>
      ...
      {
        selectedProduct && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 backdrop-blur-3xl animate-in fade-in duration-300" onClick={() => setSelectedProduct(null)}>
            <button className="absolute top-8 right-8 text-stone-500 hover:text-white transition-colors" onClick={() => setSelectedProduct(null)}>
              <X size={48} />
            </button>
            <div className="max-w-5xl w-full mx-4 grid lg:grid-cols-2 gap-12 items-center" onClick={e => e.stopPropagation()}>
              <div className="relative rounded-[2rem] overflow-hidden shadow-2xl group">
                <img src={selectedProduct.img} alt={selectedProduct.name} className="w-full h-auto object-cover" />
              </div>
              <div className="space-y-8 p-6">
                <div>
                  <h3 className="text-3xl md:text-5xl font-serif font-bold text-white mb-4 leading-tight">{selectedProduct.name}</h3>
                  <p className="text-lg text-stone-300 font-light leading-relaxed border-l-4 border-amber-600 pl-6">{selectedProduct.desc}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Auth Modal */}
      {
        isAuthModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-4 md:p-10 animate-in fade-in duration-500 overflow-y-auto">
            <div className="bg-stone-900 rounded-[3rem] p-10 w-full max-w-2xl shadow-2xl border border-white/5 my-auto text-center">
      ...
      {/* Report Reader Modal */}
      {selectedReport && (
        <div className="fixed inset-0 z-[75] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-4 md:p-10 animate-in fade-in">
          <div className="bg-stone-900 rounded-[2rem] p-0 w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl relative overflow-hidden">
            <button onClick={() => setSelectedReport(null)} className="absolute top-8 right-8 z-10 p-2 bg-black/50 rounded-full text-white hover:bg-amber-600 transition-colors"><X size={24} /></button>
            ...
            <div className="p-10 md:p-14 relative">
              {!selectedReport.image_url && <span className="text-amber-600 text-xs font-bold uppercase tracking-widest mb-4 block">Vital Core Health Essay</span>}

              <h2 className="text-3xl md:text-5xl font-serif font-bold text-white mb-6 leading-tight">{selectedReport.title}</h2>

              {selectedReport.key_point && (
                <div className="mb-10 p-6 bg-amber-500/10 border-l-4 border-amber-500 rounded-r-xl">
                  <h5 className="text-amber-500 font-bold text-xs uppercase mb-2 flex items-center gap-2"><Sparkles size={12} /> Key Point</h5>
                  <p className="text-xl md:text-2xl font-serif text-amber-100 italic leading-normal">"{selectedReport.key_point}"</p>
                </div>
              )}

              <div className="prose prose-invert prose-lg max-w-none text-stone-300 font-light leading-loose whitespace-pre-wrap">
                {selectedReport.content}
              </div>

              <div className="mt-12 pt-12 border-t border-white/5 text-center">
                <p className="text-stone-600 text-sm">Vital Core Premium Lab</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div key={q.id} className="bg-stone-900/50 p-8 rounded-[2rem] border border-white/5 hover:border-amber-500/30 transition-all">
                ...
                {!q.answer && isAdmin && (
                  <div className="mt-4 pt-4 border-t border-white/5">
                    <input
                      className="w-full bg-black/30 p-3 rounded-lg text-white text-sm border border-white/5 mb-2"
                      placeholder="Write answer..."
                      onKeyDown={async (e) => {
                        if (e.key === 'Enter') {
                          await api.qna.answer(q.id, e.currentTarget.value);
                          fetchQuestions();
                        }
                      }}
                    />
                    <p className="text-[10px] text-stone-500">Press Enter to submit answer</p>
                  </div>
                )}
              </div>
            ))}
